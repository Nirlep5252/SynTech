...
class images(commands.Cog, description="This is where you can get images"):
    ...
    async def horny(self, ctx, member: discord.Member = None):
     ...
     async with aiohttp.ClientSession() as session:
        ...
        f'https://some-random-api.ml/canvas/horny?avatar={member.avatar.replace(format="png")}') as af:
    ...
    async def gay(self, ctx, member: discord.Member = None):
     ...
     async with aiohttp.ClientSession() as session:
        async with session.get(
        f'https://some-random-api.ml/canvas/gay?avatar={member.avatar.replace(format="png")}') as af:
         if 300 > af.status >= 200:
            fp = io.BytesIO(await af.read())
    ...
    async def jail(self, ctx, member: discord.Member = None):
     ...
     async with aiohttp.ClientSession() as session:
        async with session.get(
        f'https://some-random-api.ml/canvas/jail?avatar={member.avatar.replace(format="png")}') as af:
         if 300 > af.status >= 200:
            fp = io.BytesIO(await af.read())
    ...
    async def comment(self, ctx, *, text=None):
      if text == None:
        ...
        async with aiohttp.ClientSession() as session:
         async with session.get(
          f'https://some-random-api.ml/canvas/youtube-comment?avatar={ctx.author.avatar.url}&username={ctx.author.name}&comment={text}') as af:
          if 300 > af.status >= 200:
            fp = io.BytesIO(await af.read())
